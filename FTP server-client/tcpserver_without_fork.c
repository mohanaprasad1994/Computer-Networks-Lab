...
main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        while(1){
            ...
            if(recv_data[0]=='C' && recv_data[1]=='W' && recv_data[2]=='D' && recv_data[3]==' '){
                send(connected,"OK",2,0);
                char buff[1024];
                getcwd(buff,1024);
                char temp[1024];
                strcpy(temp,&recv_data[4]);
                if(recv_data[4]=='~' || recv_data[4]=='/'){
                    strcpy(temp,root);
                    temp[strlen(root)]='/';
                ...
                if(recv_data[0]=='R' && recv_data[1]=='E' && recv_data[2]=='T' && recv_data[3]=='R' && recv_data[4]==' '){
                    printf("RETR\n");
                    fflush(stdout);
                    FILE * fil;int read;
                    if(!(fil=fopen(&recv_data[5],"r")))
                ...
                if(recv_data[0]=='S' && recv_data[1]=='T' && recv_data[2]=='O' && recv_data[3]=='R' && recv_data[4]==' '){
