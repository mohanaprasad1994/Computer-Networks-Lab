...
/////// Photo not copying properly (size check and pass)
/////// no fork
/*
...
main(int argc, char *argv[]) {
    ...
    while(1){
        ...
        if (send_data[0]=='c' && send_data[1]=='d' && send_data[2]==' '){ // for handling cd
           char temp[1024];
           strcpy(temp,"CWD ");
           ...
           if(bytes_received==0 ){
              ...
              bytes_received=recv(sock,recv_data,1024,0);
              recv_data[bytes_received]='\0';
              if(strcmp(recv_data,"Illegal CWD Operation")==0)
        ...
        if(send_data[0]=='p' && send_data[1]=='w' && send_data[2]=='d' ){ // for handling pwd. Prints both CWD of server and client
            char buff[1024];
            getcwd(buff,1024);
        ...
        if ( send_data[0]=='g' && send_data[1]=='e' && send_data[2]=='t' && send_data[3]==' '){ // Handles get requests
           char temp[1024];
           strcpy(temp,"RETR ");
           ...
           if(bytes_received==0 ){
               ...
               while(flag==0){
                   ...
                   int i;
                   //if(sz<=10000)
                       for (i=0;i<bytes_received-1 && flag==0;i++)          // recieve till end of file is reached
        ...
        if(send_data[0]=='p' && send_data[1]=='u' && send_data[2]=='t' && send_data[3]==' '){
                    FILE * fil;
                    if(!(fil=fopen(&send_data[4],"r")))
                       ...
                       else if(strcmp(recv_data,"OK to SEND File")!=0){
                           //printf("%s\n",recv_data);
