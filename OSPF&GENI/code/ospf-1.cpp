...
const int INF = 1<<30;
std::mutex mtx;
volatile int  adjacency_matrix[500][500];
...
void *listen(void *threadid)
{
  ...
  cerr << "Exiting listen" << endl;
   pthread_exit(NULL);
}
void *lsa(void *threadid)
{
	while (get_time() < time_out){
		usleep(lsai);
  ...
  cerr << "Exiting LSA " << endl;
   pthread_exit(NULL);
}
void *graph(void *threadid)
{
  int i,j;
	while (get_time() < time_out){
		usleep(spfi);
  ...
  fil<<"Routing table for Node Id "<<node_id<<" Time = "<<(get_time()- start_time)*1.0/1000000<<" seconds"<<endl;
...
int main (int argc, char *argv[])
{
  ...
  rc = pthread_create(&HELLO, NULL,hello, (void *)&i);
  if (rc){
     //cout << "Error:unable to create thread hello," << rc << endl;
     exit(-1);
  }

  rc = pthread_create(&LISTEN, NULL,listen, (void *)&i);
  if (rc){
     //cout << "Error:unable to create thread hello," << rc << endl;
     exit(-1);
  }
  rc = pthread_create(&GRAPH, NULL,graph, (void *)&i);
  if (rc){
     //cout << "Error:unable to create thread hello," << rc << endl;
     exit(-1);
  }

  rc = pthread_create(&LSA, NULL,lsa, (void *)&i);
  if (rc){
     //cout << "Error:unable to create thread hello," << rc << endl;
     exit(-1);
  }
